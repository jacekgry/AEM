...
def mutate(parent1, parent2):
    ...
    for vertex in parent1:
        if vertex in parent2:
            child.append(vertex)
    rest_points_from_parents = [x for x in parent1 if x not in child]
    rest_points_from_parents += [x for x in parent2 if x not in child]
    k = cycle_size - len(child)
    child += random.sample(rest_points_from_parents, k=k)
