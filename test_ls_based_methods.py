...
from aem import get_path_length, adjacency_matrix, problem, \
    instance_name, save_graph, MSLS, ILS1, ILS2, cycle_size, hybrid, genetic2
...
results_file = open(f'results_genetic_{instance_name}.csv', 'w')
...
print()
print(f"AVG NO OF LS,{','.join(map(lambda x: str(np.mean(no_of_ls[x.__name__])), methods))}")
print(f"MIN NO OF LS,{','.join(map(lambda x: str(np.min(no_of_ls[x.__name__])), methods))}")
print(f"MAX NO OF LS,{','.join(map(lambda x: str(np.max(no_of_ls[x.__name__])), methods))}")
